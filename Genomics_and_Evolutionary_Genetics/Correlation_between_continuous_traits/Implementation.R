...
linear_regression = function(values1, values2) {
    ...
    # ???
    res <- lm(trait2 ~ trait1, data.frame(trait1 = values1, trait2 = values2))
    s <- summary(res)
...
calculate_corrected_branch_length = function(node, child1, child2, corrected_branch_lengths) {
    ...
    # ???
    child1_length <- corrected_branch_lengths[child1]
    child2_length <- corrected_branch_lengths[child2]
    corrected_length <- ((child1_length * child2_length)/ (child1_length + child2_length)) + corrected_branch_lengths[node]
...
calculate_trait_and_contrast = function(child1, child2, traits, corrected_branch_lengths) {
    ...
    # ???
    trait_value <- calculate_trait_at_internal_node(trait_child1 = traits[[child1]], trait_child2 = traits[[child2]], 
                                                    branch_length_to_child1 = corrected_branch_lengths[[child1]], 
                                                    branch_length_to_child2 = corrected_branch_lengths[[child2]])
    
    contrast <- calculate_contrast(trait_child1 = traits[[child1]], trait_child2 = traits[[child2]], 
                                   branch_length_to_child1 = corrected_branch_lengths[[child1]], 
                                   branch_length_to_child2 = corrected_branch_lengths[[child2]])
...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    # Here we used the function given above to check if the current node is a tip in the tree
    # ???
    if (is_tip(node,tree)) {
      ...
      description <- get_contrasts_in_subtree(node = children[[1]], tree, description)
      description <- get_contrasts_in_subtree(node = children [[2]], tree, description)
      
      # Update the branch length leading to the given node, save it in the description
      # ???
      corrected_length <- calculate_corrected_branch_length(node, child1 = children[[1]], child2 = children[[2]], 
                                                                                    corrected_branch_lengths = description$corrected_branch_lengths)
      description$corrected_branch_lengths[node] <- corrected_length
      
      # Compute the trait values and normalized contrasts for both traits, save the values in the description
      # 1. First save the values for trait 1 and trait 2 into a vector
      # ???
      trait1_values <- description$traits[,"value_of_trait1"]
      trait2_values <- description$traits[,"value_of_trait2"]
      
      # 2. Then find the trait value and contrast at the current node for both trait 1 and 2 
      trait_and_contrast_1 <- calculate_trait_and_contrast(child1 = children[[1]], child2 = children[[2]], traits = trait1_values, 
                                                           corrected_branch_lengths = description$corrected_branch_lengths)
      trait_and_contrast_2 <- calculate_trait_and_contrast(child1 = children[[1]], child2 = children[[2]], traits = trait2_values, 
                                                           corrected_branch_lengths = description$corrected_branch_lengths)
      
      # 3. Save the values in the description
      description$traits[node,"value_of_trait1"] <- trait_and_contrast_1$trait_value
      description$traits[node,"value_of_trait2"] <- trait_and_contrast_2$trait_value
      description$normalized_contrasts[node, "normalized_contrast_trait1"] <- trait_and_contrast_1$contrast
      description$normalized_contrasts[node, "normalized_contrast_trait2"] <- trait_and_contrast_2$contrast
...
get_trait_correlation = function(newick_tree, traits_at_tip) {	
    ...
    # Use the provided get_root function to find the index of the root note, then use the get_contrasts_in_subtree function
    # to find all the trait values, normalized contrasts and corrected branch lengths, by moving down the tree.
    # ???
    root <- get_root(tree)
    updated_description <- get_contrasts_in_subtree(node = root, tree, description)

    # Perform linear regression on the traits at the tips and get the slope, intercept, P-value, and R2-value
    # Use the linear_regression function
    # ???
    raw_data_regression <- linear_regression(values1 = traits_at_tip$trait1, values2 = traits_at_tip$trait2)

    # Perform linear regression on the normalized contrasts and get the slope, intercept, P-value, and R2-value
    # ???
    contrasts_regression <- linear_regression(values1 = updated_description$normalized_contrasts[,"normalized_contrast_trait1"], 
                                              values2 = updated_description$normalized_contrasts[,"normalized_contrast_trait2"])
